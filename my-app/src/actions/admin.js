...
export async function loginAdmin(prevState, formData) {
  try {
    ...
    console.log("hello3", user._id.toString());
    return {
      success: true,
 ...
 */
export async function createInterviewer(formData) {
  try {
    // Verify admin auth
    ...
    const role = "INTERVIEWER"; // formData.get("role") || "interviewer";
...
export async function updateUser(prevState, formData) {
  try {
    ...
    if (auth.role !== "superadmin") {
      if (
        userToUpdate.role !== "interviewer" ||
        (role && role !== "interviewer")
      ) {
        return {
          success: false,
          message:
            "You do not have permission to update admin accounts or change roles",
        };
      }
    }
    ...
    // Update user
    const updatedUser = await Interviewer.findByIdAndUpdate(id, updateData, {
      new: true,
    }).select("-password");
...
export async function getAuthAdmin() {
  try {
    ...
    const admin = await Interviewer.findById({ id: userId }).select(
      "-password"
    );
